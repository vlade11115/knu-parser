...
class ScheduleSpider(Spider):
    ...
    start_urls = ["http://asu.knu.edu.ua/timeTable/group"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        end_time = parse(
            getattr(self, "end_time", str(start_date + datetime.timedelta(weeks=8)))
        )
        self.dates = {
    ...
    def parse_course(self, response):
        ...
        Parse id of course.
        """
        for course_id in response.xpath(
            '//*[@id="TimeTableForm_course"]/option/@value'
        )[1:].getall():
    ...
    def parse_schedule(self, response):
        ...
        for row in table:
            ...
            lessons_info = row.xpath(
                './td/div[@class="mh-50 cell cell-vertical"]/span[1]/text()'
            ).getall()
            lessons_start = row.xpath(
                './td/div[@class="mh-50 cell cell-vertical"]/span[@class="start"]/text()'
            ).getall()
            lessons_finish = row.xpath(
                './td/div[@class="mh-50 cell cell-vertical"]/span[@class="finish"]/text()'
            ).getall()
            ...
            for index, day in enumerate(days):
                ...
                for lesson in lessons:
                    ...
                    if not lesson_tag_value:
                        ...
                        continue
                    lesson = list(
                        filter(None, map(str.strip, lesson_tag_value.split("<br>")))
                    )
                    discipline = lesson[0]
                    item["discipline"] = discipline[: discipline.find("[")]
                    item["lesson_type"] = discipline[
                        discipline.find("[") + 1 : discipline.find("]")
                    ]
